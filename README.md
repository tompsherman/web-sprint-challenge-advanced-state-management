...
It allows for global state management but on a component level, so you don't have to prop drill props from the top all the way down.

2. In your own words, describe `actions`, `reducers` and the `store` and their role in Redux. What does each piece do? Why is the store known as a 'single source of truth' in a redux application?

- the store contains all state/state management, and is the single source of truth because redux apps/components are immutable, so teh store will contain the entire state at the current point in time.
- reducers are like return statements, they are triggered by specific actions and then update state.
- actions can be any action undertaken on page (click, mouse in, etc.) and trigger the reducer to update state in order to change the omponent.

3. What is the difference between Application state and Component state? When would be a good time to use one over the other?

Application state gives access to state across teh hwole application, whereas component state is specific to a single component. if the state is only used in a single component, component state is preferable, but if staet data iis needed across multiple components within the application, applciation state is preferable

4. Describe `redux-thunk`, what does it allow us to do? How does it change our `action-creators`?

thunk is a function inside a function, allows us to catch an action triggered and determine what type of data it is and how to process is (f an object it can carry on , but if a function must me invoked before continuing through the redux workflow)

5. What is your favorite state management system you've learned and this sprint? Please explain why!

context APIs are simpler and certainly interesting, i do enjoy redux after wrapping my mind around it.
